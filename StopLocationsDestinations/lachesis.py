...
def process_data(df, roam_dist, min_stay, n_jobs, print_output=None):
    """
    ...
    '''
    df_stops = [process_user(np.array(df.loc[userids[i], ["latitude", "longitude"]].reset_index()),
         i, userids[i], n_users, min_stay, roam_dist, print_output) 
             for i in range(n_users)]
    '''
    
    
    df_stops = Parallel(n_jobs=n_jobs, verbose=10)\
