...
class ModelingProcessing(object):
    ...
    def model(self):
        ...
        '''
        #print("===================== COLUMNAS ===============")
        #print(list(self.df_seg.columns.values))
        #print(' ')
        
        # ============== Reemplazamos valores muy pequeños por cero =========
        print(type(self.df_seg))
        
        mask = self.df_seg.vel_low_rate < 0.001
        column_name = 'vel_low_rate'
        self.df_seg.loc[mask, column_name] = 0
        
        mask = self.df_seg.vel_change_rate < 0.001
        column_name = 'vel_change_rate'
        self.df_seg.loc[mask, column_name] = 0
        
        mask = self.df_seg.bearing_change_rate < 0.001
        column_name = 'bearing_change_rate'
        self.df_seg.loc[mask, column_name] = 0
        '''
        ...
        #print(y_train)
        #print(' ')
        #print(np.isinf(X_train_final.values).any())
        #X_train_final = self.clean_dataset(X_train_final)
        #print(X_train_final[feature_cols].dtypes)
...
if __name__ == "__main__":
    mp = ModelingProcessing("/home/jordi/Desktop/UNI/TFG/DT-RF-LR-SVM-KNN/segment_featured_master.csv")
